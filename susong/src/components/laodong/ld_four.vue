<template>
  <div id="box">
    ...
    <p>{{title}}</p>
    <!-- 单选题选择 -->
    <el-radio-group v-model="selectionsList[3].value" @change="changeValue">
...
export default {
  data() {
    return {
      ...
      radio1: "",
      one: "因确认劳动关系发生争议",
      two: "因劳动报酬、工伤医疗费、经济补偿或者赔偿金等发生的争议",
      three: "因工作时间、休息休假、社会保险、福利、培训以及劳动保护发生的争议",
      four:"因除名、辞退和辞职、离职或订立、履行、变更、解除和终止劳动合同发生的争议",
      five:"其他",
      score: "",
      value: ""
    };
  ...
  methods: {
    changeValue(e){
      ...
      let _this=this
      setTimeout(function(){
        _this.$router.push({path:'/ld_five'})
      },300)
    },
    v_click(){
