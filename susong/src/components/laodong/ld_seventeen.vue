<template>
  <div id="box">
    ...
    <p>{{title}}</p>
    <!-- 单选题选择 -->
    <el-radio-group v-model="selectionsList[16].value" @change="changeValue">
...
export default {
  ...
  created(){
    this.value=parseInt(localStorage.getItem("key16"))
  },
  mounted() {
  ...
  methods: {
    changeValue(e){
      ...
      console.log(this.score)
      localStorage.setItem('key17',this.score)
      
      let _this=this
      setTimeout(function(){
        _this.$router.push({path:'/ld_eighteen'})
      },300)
    },
    v_click(){
