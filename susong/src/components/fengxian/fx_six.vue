<template>
  <div id="box">
    ...
    <p>{{title}}</p>
    <!-- 单选题选择 -->
    <el-radio-group v-model="selectionsList[5].value" @change="changeValue">
...
export default {
  data() {
    return {
      title: "不准确提供送达地址，将会导致",
      radio1: "",
      one: "简易程序审理的案件，因地址不准确造成诉讼文书被退回，诉讼文书也视为送达",
      two: "将会重复送达，直到当事人受到该信息",
      score: "",
      value: ""
    };
  },
  ...
  methods: {
    changeValue(e){
      ...
      console.log(this.score)
      localStorage.setItem('key6',this.score)
      
      let _this=this
      setTimeout(function(){
        _this.$router.push({path:'/fx_seven'})
      },300)
    },
    v_click(){
