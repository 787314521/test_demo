<template>
  <div id="box">
    ...
    <p>{{title}}</p>
    <!-- 单选题选择 -->
    <el-radio-group v-model="selectionsList[9].value" @change="changeValue">
...
export default {
  data() {
    return {
      title: "其他财产的具体情况是？",
      radio1: "",
      one: "清楚掌握家庭财产情况切有相应证据予以证明",
      two: "对想分割的财产无法提供证据证明具体情况",
      three: "我并不清楚掌握全部可分割的财产情况，仅能提供线索",
      score: "",
      value: ""
    };
  },
  created(){
    this.value=parseInt(localStorage.getItem("key9"))
  },
  mounted() {
  ...
  methods: {
    changeValue(e){
      ...
      console.log(this.score)
      localStorage.setItem('key10',this.score)
      
      let _this=this
      setTimeout(function(){
        _this.$router.push({path:'/hy_eleven'})
      },300)
    },
    v_click(){
