...
export default {
  data() {
    return {
      title: "您是否具有以下申请诉前保全的条件？",
      one: "因情况紧急，不立即申请保全将会使我的合法权益受到难以弥补的损害",
      two: "我申请诉前保全的时间是在向人民法院提起诉讼前",
      three: "我是利害关系人",
      four: "我提供了担保（物的担保、现金担保、人的担保、保险担保）",
      five: "没有以上情况",
      checkList: [],
      value: 0,
      score1: "",
  ...
  methods: {
    ...
    change(e) {
        console.log('e:',e)
        const isNoEvidence = e.some(v => v == 5)
    ...
    isOk() {
      ...
      localStorage.setItem("key3", this.value + this.score);

      if (this.checkList != "") {
        this.$router.push({ path: "/sq_four" });
      } else {
        this.$alert("请选择选项！", "提示", {
...
/deep/ .one_box .el-checkbox__label {
  top: 0.1rem;
}
