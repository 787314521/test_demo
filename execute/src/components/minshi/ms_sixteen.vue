<template>
  <div id="box">
    ...
    <p>{{title}}</p>
    <!-- 单选题选择 -->
    <el-radio-group v-model="selectionsList[15].value" @change="changeValue" @click.native="s_click">
...
export default {
  ...
  created() {
    this.value = parseInt(localStorage.getItem("key15"));
  },
  mounted() {
  ...
  methods: {
    changeValue(e) {
      ...
      localStorage.setItem("key16", this.score + this.value);

      let _this = this;
      clearTimeout(timer);
      timer = setTimeout(function() {
        _this.$router.push({ path: "/ms_seventeen" });
      }, 300);
    },
    s_click() {
      let _this = this;
