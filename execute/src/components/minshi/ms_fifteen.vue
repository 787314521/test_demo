<template>
  <div id="box">
    ...
    <p>{{title}}</p>
    <!-- 单选题选择 -->
    <el-radio-group v-model="selectionsList[14].value" @change="changeValue" @click.native="s_click">
...
export default {
  data() {
    return {
      title: "上述钱、车、房屋等物是否有明确金额，牌号车型，地址等信息",
      radio1: "",
      one: "有明确支付金额，牌号车型，房屋详细地址等信息",
      two: "各信息尚不明确",
      score: 0,
      value: ""
    };
  ...
  created() {
    this.value = parseInt(localStorage.getItem("key14"));
  },
  mounted() {
  ...
  methods: {
    changeValue(e) {
      ...
      localStorage.setItem('key15',this.score+this.value)
