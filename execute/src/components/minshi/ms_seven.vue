<template>
  <div id="box">
    ...
    <p>{{title}}</p>
    <!-- 单选题选择 -->
    <el-radio-group v-model="selectionsList[6].value" @change="changeValue" @click.native="s_click">
...
export default {
  data() {
    return {
      title: "您申请执行所依据的法律文书符合以下哪个时效范围",
      radio1: "",
      one: "法律文书规定履行期间的最后一日起两年内",
      two: "法律文书规定履行期间的最后一日起，已超过两年",
      score: 0,
      value: ""
    };
  },
  ...
  methods: {
    changeValue(e) {
      ...
      localStorage.setItem('key7',this.score+this.value)
      
      let _this=this
      clearTimeout(timer)
      timer=setTimeout(function(){
        _this.$router.push({path:'/ms_eight'})
      },300)
    },
     s_click(){
      let _this = this;
