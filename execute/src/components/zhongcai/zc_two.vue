<template>
  <div id="box">
    ...
    <p>{{title}}</p>
    <!-- 单选题选择 -->
    <el-radio-group v-model="selectionsList[1].value" @change="changeValue" @click.native="s_click">
...
export default {
  data() {
    return {
      title: "您申请执行的法院是",
      radio1: "",
      one: "被执行人住所地或被执行的财产所在地中级人民法院",
      two: "被执行人住所地或被执行的财产所在地基层人民法院",
      three: "其他法院",
      score: 0,
      value: ""
    };
  ...
  methods: {
    changeValue(e) {
      ...
      localStorage.setItem("key2", this.score + this.value);

      let _this = this;
      clearTimeout(timer);
      timer = setTimeout(function() {
        _this.$router.push({ path: "/zc_three" });
      }, 300);
    },
    s_click() {
      let _this = this;
