<template>
  <div id="box">
    ...
    <p>{{title}}</p>
    <!-- 单选题选择 -->
    <el-radio-group v-model="selectionsList[9].value" @change="changeValue" @click.native="s_click">
...
export default {
  ...
  methods: {
    changeValue(e) {
      ...
      localStorage.setItem("key10", this.score + this.value)

      let _this = this
      clearTimeout(timer)
      timer = setTimeout(function() {
        _this.$router.push({ path: "/xy_eleven" })
      }, 300)
    },
    s_click() {
      let _this = this
