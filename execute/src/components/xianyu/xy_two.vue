...
export default {
  data() {
    return {
      title: "您具有下列哪些申请先予执行的条件",
      one: "在受理案件后终审判决作出前提出申请",
      two: "当事人之间的权利义务关系明确",
      three: "不先予执行将会影响申请人的生活或者生产经营",
      four: "被执行人有履行能力",
      five: "我提供了担保（物的担保、现金担保、人的担保、保险担保）",
      six: "没有上述各类条件",
      checkList: [],
      value: 0,
      score1: "",
  ...
  methods: {
    ...
    change(e) {
        console.log('e:',e)
        const isNoEvidence = e.some(v => v == 6)
    ...
    isOk() {
      ...
      if (this.checkList != "") {
        this.$router.push({ path: "/xy_three" });
      } else {
        this.$alert("请选择选项！", "提示", {
